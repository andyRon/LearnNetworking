...
![](images/NetworkProtocol-27-01.jpg)
...
第一大类称为**无类别排队规则**（Classless Queuing Disciplines）。还记得我们讲[ip addr](NP-3.md)的时候讲过的**pfifo_fast**，这是一种不把网络包分类的技术。

![](images/NetworkProtocol-27-02.jpg)

pfifo_fast 分为三个先入先出的队列，称为三个Band。根据网络包里面 TOS，看这个包到底应该进入哪个队列。TOS 总共四位，每一位表示的意思不同，总共十六种类型。
...
![](images/NetworkProtocol-27-05.jpg)
...
![](images/NetworkProtocol-27-06.jpg)

首先，在 port 上可以创建 QoS 规则，一个 QoS 规则可以有多个队列 Queue。

![](images/NetworkProtocol-27-07.jpg)
...
- 在云中网络Openvswitch中，主要使用的是**分层令牌桶规则（HTB）**，将总的带宽在一棵树上按照配置的比例进行分配，并且在一个分支不用的时候，可以借给另外的分支，从而增强带宽利用率。
